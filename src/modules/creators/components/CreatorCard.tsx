      ...
      <Card className="glass-card flex flex-col h-full overflow-hidden transition-all duration-500 hover:translate-y-[-6px] hover:shadow-2xl hover:border-primary/40 relative">
        <CardHeader className="p-6 pb-2 flex flex-row items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <Text variant="caption" className="text-primary font-bold uppercase tracking-widest text-[9px] mb-1 block">
              {creator.title}
            </Text>
        ...
        <CardContent className="p-6 pt-2 flex-grow">
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {creator.badges?.slice(0, 2).map(badge => (
              <Badge key={badge} variant="outline" className="border-secondary/20 bg-secondary/5 text-secondary text-[8px] font-bold uppercase h-5 px-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {creator.specialties.slice(0, 3).map(spec => (
              <Badge key={spec} variant="secondary" className="bg-primary/5 text-primary text-[9px] font-bold uppercase tracking-tighter border-none">
                {spec}
              </Badge>
            ))}
          </div>
