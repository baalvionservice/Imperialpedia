...
import { getEditorialDashboardData } from '@/services/api/editorial'; // Corrected path or use mock
  ...
  const getActionBadge = (action: EditorialLogEntry['action']) => {
    switch (action) {
      ...
      case 'Revision Requested':
        return <Badge className="bg-amber-500/10 text-amber-500 border-none font-bold uppercase text-[8px] h-5 px-2">Revision</Badge>;
      case 'Article Rejected':
        return <Badge variant="destructive" className="font-bold uppercase text-[8px] h-5 px-2">Rejected</Badge>;
      default:
        return <Badge variant="outline" className="text-[8px] font-bold uppercase h-5 px-2">Reassigned</Badge>;
    }
  };
    ...
    <div className="space-y-10 pb-24 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-primary mb-1">
              ...
              <Text variant="label" className="text-[10px] font-bold tracking-widest uppercase">Audit Trail</Text>
            </div>
            <Text variant="h1" className="text-3xl font-bold tracking-tight">Editorial Lifecycle Log</Text>
          </div>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-500">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest">Log Integrity Verified</span>
        </div>
      </header>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/20 border-b border-white/5">
                ...
                <TableHead className="font-bold text-[10px] uppercase tracking-widest">Administrative Verdict</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest">Intelligence Title</TableHead>
                <TableHead className="text-right pr-8 font-bold text-[10px] uppercase tracking-widest">Handshake ID</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="h-48 text-center text-muted-foreground italic">
                    No matching activity records localized in the current search buffer.
                  </TableCell>
                </TableRow>
              ) : filtered.map((log) => (
                <TableRow key={log.id} className="group hover:bg-white/5 transition-colors border-b border-white/5">
                  <TableCell className="py-5 pl-8">
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-foreground/70">{format(new Date(log.timestamp), 'MMM d, yyyy')}</span>
                      <span className="text-[9px] text-muted-foreground font-mono uppercase">{format(new Date(log.timestamp), 'HH:mm:ss')} UTC</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-medium">{log.editor_name}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <span className="text-xs font-bold text-foreground/80 truncate max-w-[250px] block" title={log.article_title}>
                      {log.article_title}
                    </span>
                  </TableCell>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="glass-card bg-primary/5 border-primary/20 p-8 flex flex-col gap-4">
          ...
          <div>
            ...
            <Text variant="bodySmall" className="text-muted-foreground leading-relaxed">
              Every decision committed by the auditor cluster is cryptographically signed and immutable. Audit trails are retained for **365 days** within the production ring for regulatory compliance.
            </Text>
          </div>
        </Card>
        ...
        <Card className="glass-card border-secondary/20 p-8 flex flex-col gap-4 relative overflow-hidden">
          ...
          <div>
            <Text variant="h3" className="mb-2 text-xl font-bold">Decision Analytics</Text>
            <Text variant="bodySmall" className="text-muted-foreground leading-relaxed">
              Analyzing historical logs: The cluster has maintained an **SLA of 99.9%** for approval handshakes. 85% of 'Approved' nodes pass through the triage layer in under 6 hours.
            </Text>
          </div>
        </Card>
      </div>
