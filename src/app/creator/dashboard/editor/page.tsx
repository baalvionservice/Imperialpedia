...
const articleSchema = z.object({
  title: z.string().min(5, 'A professional headline requires at least 5 characters.'),
  snippet: z.string().min(20, 'Provide a substantial executive summary (20+ chars).'),
  content: z.string().min(100, 'Intelligence insights must be thorough (100+ chars).'),
...
export default function CreatorEditorPage() {
  ...
  const onPublish = async (values: ArticleFormValues) => {
    ...
    toast({
      ...
      description: "Your analysis is now live and indexed for platform-wide reach.",
    });
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" className="rounded-full" asChild>
            <Link href="/creator/dashboard"><ArrowLeft className="h-5 w-5" /></Link>
          </Button>
          <div>
            <div className="flex items-center gap-2 text-primary mb-1">
              <FileText className="h-4 w-4" />
              <Text variant="label" className="text-[10px] font-bold tracking-widest uppercase">Intelligence Studio</Text>
            </div>
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={handleSaveDraft} disabled={isSaving || isPublishing} className="h-11 px-6 rounded-xl border-white/10 bg-card/30">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button disabled={isPublishing} className="h-11 px-8 rounded-xl bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20 font-bold">
            ...
            <AlertDialogContent className="glass-card border-primary/20">
              <AlertDialogHeader>
                <AlertDialogTitle className="text-2xl font-bold">Commit to the Intelligence Index?</AlertDialogTitle>
                <AlertDialogDescription className="text-muted-foreground">
                  This will finalize your research and make it available to the global network. Ensure all claims are verified against primary sources.
                </AlertDialogDescription>
              </AlertDialogHeader>
              ...
              <Card className="glass-card border-none shadow-2xl">
                <CardHeader className="bg-card/30 border-b border-white/5 pb-6">
                  <div className="flex items-center gap-2 text-primary mb-2">
                    ...
                    <Text variant="caption" className="font-bold uppercase tracking-wider">Expert Analysis</Text>
                  </div>
                  <FormField
                ...
                <CardContent className="p-8 space-y-8">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Executive Summary</FormLabel>
                        <FormControl>
                          <Textarea 
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">Deep-Dive Analysis</FormLabel>
                        <FormControl>
                          <Textarea 
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" variant="outline" size="icon" className="h-11 w-11 rounded-xl" onClick={handleAddTag}>
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
                <div className="flex flex-wrap gap-2 pt-2">
                  {tags.map((tag) => (
                    <Badge key={tag} variant="secondary" className="pl-3 pr-1.5 py-1.5 gap-1.5 bg-secondary/10 text-secondary border-none rounded-lg text-[10px] font-bold">
                      {tag}
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Professional Headline
                </div>
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Executive Summary Ready
                </div>
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
              ...
              <div className="pt-4 border-t border-white/5">
                <Text variant="caption" className="italic text-muted-foreground leading-relaxed">
                  "Expert insights with at least 3 Topic Nodes and an Executive Summary receive 2.4x more organic reach."
                </Text>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="p-6 rounded-[2rem] border border-secondary/20 bg-secondary/5 space-y-4">
            ...
            <Text variant="caption" className="text-muted-foreground leading-relaxed">
              We are currently prioritizing intelligence on **Central Bank Liquidity Cycles**. Insights in the 'Economics' hub with related nodes are being featured in the primary Intelligence Feed.
            </Text>
          </div>
        </div>
      </div>
