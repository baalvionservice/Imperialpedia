      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-card/30 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl backdrop-blur-sm">
        <div className="flex-1 space-y-4">
          <div className="flex items-center gap-2 text-primary">
            ...
            <Text variant="label" className="text-[10px] font-bold tracking-widest uppercase">Institutional Library</Text>
          </div>
          ...
          <Card key={report.id} className={cn(
            ...
            <CardHeader className="bg-card/30 border-b border-white/5 p-6 flex flex-row items-center justify-between">
              <div>
                ...
                <CardTitle className="text-xl font-bold group-hover:text-primary transition-colors">{report.report_name}</CardTitle>
              ...
              <div className="flex gap-2">
                <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-primary transition-colors" onClick={() => handleDownload(report.report_name)}>
            ...
            <CardContent className="p-8">
              <Text variant="bodySmall" className="text-muted-foreground mb-8 leading-relaxed italic border-l-2 border-primary/20 pl-4 py-1">
                "{report.description}"
              </Text>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent border-white/5 bg-muted/20">
                        <TableHead className="text-[10px] font-bold uppercase tracking-widest pl-6">Sector Node</TableHead>
                        <TableHead className="text-[10px] font-bold uppercase tracking-widest text-right">Yield Var.</TableHead>
                        <TableHead className="text-[10px] font-bold uppercase tracking-widest text-right pr-6">Risk Profile</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {report.data.map((row, i) => (
                        <TableRow key={i} className="border-white/5 hover:bg-white/5 transition-colors">
                          <TableCell className="font-bold text-sm pl-6">{row.sector}</TableCell>
                          <TableCell className={cn(
                            "text-right font-mono font-bold",
                            row.performance.includes('+') ? "text-emerald-500" : "text-destructive"
                          )}>{row.performance}</TableCell>
                          <TableCell className="text-right pr-6">
                            <Badge variant="secondary" className="text-[8px] font-bold uppercase border-none bg-muted/30">
                              {row.volatility}
                            </Badge>
                          </TableCell>
                        </TableRow>
              ...
              {report.type === 'summary' && (
                <div className="space-y-4">
                  {report.data.map((node, i) => (
                    <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-background/50 border border-white/5 group/item hover:border-secondary/30 transition-all shadow-inner">
                      ...
                      <div className="text-right">
                        ...
                        <Text variant="caption" className="text-muted-foreground text-[10px] uppercase font-bold tracking-tighter">Verified Node</Text>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <CardFooter className="bg-muted/10 border-t border-white/5 p-4 flex justify-between items-center group-hover:bg-primary/5 transition-colors">
              <Text variant="caption" className="text-muted-foreground font-medium flex items-center gap-1.5">
                <ShieldCheck className="h-3 w-3 text-emerald-500" /> E-E-A-T Verified Audit
              </Text>
              <Button variant="link" className="p-0 h-auto text-primary text-[10px] font-bold uppercase tracking-widest group/link" asChild>
                <a href="#">
                  Analyze Research Node <ArrowRight className="ml-1.5 h-3 w-3 transition-transform group-hover/link:translate-x-1" />
                </a>
              </Button>
            </CardFooter>
          </Card>
      ...
      <Card className="glass-card border-none bg-primary/5 p-12 relative overflow-hidden text-center lg:text-left">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 relative z-10">
          ...
          <div className="flex-1 space-y-3">
            <Text variant="h2" className="text-3xl font-bold tracking-tight">Custom Institutional Audits</Text>
