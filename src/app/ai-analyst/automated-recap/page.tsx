    ...
    <main className="min-h-screen bg-background pb-20 pt-12">
      <Container>
        <header className="mb-16 max-w-3xl mx-auto text-center space-y-6">
          ...
          <Text variant="h1" className="text-4xl lg:text-6xl font-bold tracking-tight">Automated Recaps</Text>
          <Text variant="body" className="text-muted-foreground text-lg leading-relaxed">
            Consolidated intelligence reports for any market cycle. Monitor top movers, laggers, and strategic takeaways in a high-fidelity retro view.
          </Text>
        </header>

        <div className="space-y-12">
          {recaps.map((recap, idx) => (
            <div key={recap.date} className="animate-in fade-in slide-in-from-bottom-4 duration-700" style={{ animationDelay: `${idx * 150}ms` }}>
              <div className="flex items-center gap-4 mb-6 px-2">
                <div className="p-3 rounded-2xl bg-primary/10 text-primary">
                  <Calendar className="h-6 w-6" />
                </div>
                <div>
                  <Text variant="h2" className="text-2xl font-bold">{format(new Date(recap.date), 'MMMM d, yyyy')}</Text>
                  <Text variant="caption" className="text-muted-foreground uppercase tracking-widest font-bold text-[9px]">Market Cycle Summary</Text>
                </div>
                  ...
                  <Card className="glass-card border-none shadow-xl bg-emerald-500/5 h-full">
                    ...
                    <CardContent className="space-y-3">
                      {recap.top_movers.map((mover, i) => (
                        <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-background/40 border border-emerald-500/10 hover:border-emerald-500/30 transition-colors">
                          <Text variant="bodySmall" className="font-bold">{mover.split(' ')[0]}</Text>
                          <span className="text-sm font-mono font-bold text-emerald-500">{mover.split(' ')[1]}</span>
                        </div>
                      ))}
                    </CardContent>
                  </Card>
                  ...
                  <Card className="glass-card border-none shadow-xl bg-destructive/5 h-full">
                    ...
                    <CardContent className="space-y-3">
                      {recap.top_laggers.map((lagger, i) => (
                        <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-background/40 border border-destructive/10 hover:border-destructive/30 transition-colors">
                          <Text variant="bodySmall" className="font-bold">{lagger.split(' ')[0]}</Text>
                          <span className="text-sm font-mono font-bold text-destructive">{lagger.split(' ')[1]}</span>
                        </div>
                      ))}
                    </CardContent>
                  </Card>
                ...
                <div className="lg:col-span-4">
                  <Card className="glass-card border-none shadow-2xl bg-primary/5 h-full relative overflow-hidden group">
                    ...
                    <CardContent className="space-y-4">
                      {recap.key_points.map((point, i) => (
                        <div key={i} className="flex gap-3 items-start group/item">
                          ...
                          <Text variant="caption" className="text-muted-foreground group-hover/item:text-foreground transition-colors leading-relaxed italic">
                            "{point}"
                          </Text>
                        </div>
                      ))}
                    ...
                    <CardFooter className="pt-0 pb-6 px-6 mt-auto">
                      <Button variant="ghost" size="sm" className="w-full text-[10px] font-bold uppercase tracking-widest text-primary hover:bg-primary/10 group/btn">
                        Full Daily Audit <ChevronRight className="ml-1 h-3 w-3 transition-transform group-hover/btn:translate-x-1" />
                      </Button>
                    </CardFooter>
                  </Card>
                </div>
        ...
        <div className="p-12 rounded-[3.5rem] bg-secondary/5 border border-secondary/20 flex flex-col lg:flex-row items-center gap-10 mt-24 relative overflow-hidden group">
          ...
          <div className="flex-1 text-center lg:text-left space-y-2 relative z-10">
            <Text variant="h2" className="text-2xl font-bold">Longitudinal Cycle Analysis</Text>
            <Text variant="bodySmall" className="text-muted-foreground leading-relaxed max-w-3xl">
              Our automated recaps aggregate data from the **Institutional Wires Hub** and **Social Sentiment Nodes**. Use these to verify if your portfolio allocation aligns with current sector-wide rotation trends.
            </Text>
          </div>
          <Button size="lg" className="h-14 px-10 rounded-2xl font-bold bg-secondary hover:bg-secondary/90 text-secondary-foreground shadow-xl shadow-secondary/30 shrink-0 relative z-10" asChild>
