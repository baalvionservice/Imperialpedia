...
export function ContestsClient({ data }: ContestsClientProps) {
  ...
  const ongoingContests = data.predictionContests.filter(c => c.status === 'Active' || c.status === 'ongoing');
  const upcomingContests = data.predictionContests.filter(c => c.status === 'Upcoming' || c.status === 'upcoming');
  const closedContests = data.predictionContests.filter(c => c.status === 'Completed' || c.status === 'closed');
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="rounded-xl h-11 px-6 border-white/10 bg-card/30" asChild>
            <Link href="/community/profile/predictions"><History className="mr-2 h-4 w-4" /> My Predictions</Link>
          </Button>
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between px-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <Text variant="h4" className="font-bold">Active Forecasts</Text>
