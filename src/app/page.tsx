      ...
      <section className="py-20 bg-card/20 border-y">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { title: 'Content Engine', desc: 'Robust publishing system for financial articles.', icon: BookOpen, color: 'text-primary' },
              { title: 'pSEO Engine', desc: 'Programmatic infrastructure for 1M+ pages.', icon: Database, color: 'text-secondary' },
              { title: 'Creator Economy', desc: 'Tools for experts to build audience and revenue.', icon: PenTool, color: 'text-primary' },
              { title: 'Financial Glossary', desc: 'Deep dictionary of market terms.', icon: ShieldCheck, color: 'text-secondary' },
              { title: 'Calculators', desc: 'Precision instruments for everyday planning.', icon: PieChart, color: 'text-primary' },
              { title: 'Analytics Hub', desc: 'Real-time platform insights.', icon: ArrowRight, color: 'text-secondary' }
            ].map((module, idx) => (
              <Card key={idx} className="glass-card transition-all hover:translate-y-[-4px]">
                <CardHeader>
