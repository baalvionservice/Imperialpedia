    ...
    <div className="space-y-10 pb-24 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <Text variant="h1" className="text-3xl font-bold tracking-tight">Analytics & Monitoring</Text>
        ...
        {data.analytics.map((kpi) => (
          <Card key={kpi.metric} className="glass-card border-none shadow-xl group hover:border-primary/20 transition-all">
            <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{kpi.metric}</CardTitle>
              {kpi.trend === 'up' ? (
            ...
            <CardContent>
              <div className="text-3xl font-bold tracking-tight">{kpi.value.toLocaleString()}</div>
              <div className={cn(
                ...
                {kpi.trend === 'up' ? 'Increasing velocity' : 'Decreasing momentum'}
              </div>
            </CardContent>
          </Card>
        ))}
            ...
            <CardHeader className="bg-card/30 border-b border-white/5 p-8 flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-xl flex items-center gap-2">
                  <Server className="h-5 w-5 text-primary" /> Infrastructure Registry
                </CardTitle>
                <CardDescription>Live health and resource consumption across production clusters.</CardDescription>
              </div>
              <Badge variant="outline" className="border-emerald-500/20 bg-emerald-500/5 text-emerald-500 text-[10px] font-bold uppercase tracking-widest px-3 h-7">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/20 hover:bg-muted/20 border-b border-white/5">
                    ...
                    <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center">CPU Usage</TableHead>
                    <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center">Memory Hub</TableHead>
                    <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center">Requests</TableHead>
                    <TableHead className="font-bold text-[10px] uppercase tracking-widest text-right pr-8">SLA Status</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {data.system_monitoring.map((node) => (
                    <TableRow key={node.server} className="group hover:bg-muted/10 transition-colors border-b border-white/5">
                      <TableCell className="py-6 pl-8">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-bold uppercase tracking-tight">{node.server}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1.5 min-w-[100px] px-4">
                          <div className="flex justify-between text-[9px] font-bold text-muted-foreground uppercase">
                            <span>Commit</span>
                            <span className="text-foreground">{node.memory_usage}</span>
                          </div>
                          <Progress value={parseInt(node.memory_usage)} className="h-1 bg-white/5" />
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-right pr-8">
                        <div className="flex flex-col items-end">
                          <span className="text-xs font-bold">{node.uptime} Uptime</span>
                          <span className={cn(
                            "text-[9px] font-bold uppercase mt-1",
                            node.status === 'healthy' ? "text-emerald-500" : 
                            node.status === 'warning' ? "text-amber-500" : "text-destructive"
                          )}>
                            {node.status}
                          </span>
                        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-white/5">
                {data.alerts.map((alert, idx) => (
                  <div key={idx} className="p-5 hover:bg-destructive/10 transition-colors space-y-3">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <Text variant="caption" className="font-bold text-foreground block">{alert.alert_type} violation</Text>
                        <Text variant="caption" className="text-muted-foreground uppercase text-[9px] tracking-tighter">Node: {alert.server}</Text>
                      </div>
                      <Badge variant="destructive" className="animate-pulse text-[8px] font-bold h-5 px-2">TRIGGERED</Badge>
                    </div>
                    <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/5">
                      <div>
                        <Text variant="label" className="text-[8px] opacity-50 block mb-1">Limit</Text>
                        <Text variant="caption" className="font-bold text-foreground">{alert.threshold}</Text>
                      </div>
                      <div className="text-right">
                        <Text variant="label" className="text-[8px] opacity-50 block mb-1">Current</Text>
                        <Text variant="caption" className="font-bold text-destructive">{alert.current}</Text>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="p-4 bg-destructive/10 border-t border-white/5">
                <Button variant="ghost" className="w-full h-10 text-[9px] font-bold uppercase tracking-widest text-destructive hover:bg-destructive/20 rounded-none">
          ...
          <Card className="glass-card border-none shadow-xl bg-primary/5">
            ...
            <CardContent>
              <Text variant="caption" className="italic text-muted-foreground leading-relaxed">
                "Infrastructure performance is currently stable within the **99.9% SLA** buffer. High CPU alerts on Server2 are correlated with the Q2 re-indexing cycle and require no immediate administrative traversal."
              </Text>
            </CardContent>
          </Card>
          ...
          <div className="p-8 rounded-[2.5rem] border border-secondary/20 bg-secondary/5 space-y-4 relative overflow-hidden group">
            ...
            <Text variant="caption" className="text-muted-foreground leading-relaxed">
              Telemetry is synchronized with the **Institutional Data Node** every 60 seconds. Any state deviation exceeding ±5% triggers a high-priority alert.
            </Text>
            <Button variant="link" className="p-0 h-auto text-secondary text-xs font-bold hover:bg-transparent" asChild>
              <Link href="/admin/control/audit-trail">Review Full Audit Trail</Link>
            </Button>
          </div>
        </div>
      </div>
