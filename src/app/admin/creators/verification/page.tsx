...
export default function CreatorVerificationPage() {
  ...
  const handleAction = (creatorId: string, action: 'approve' | 'reject') => {
    ...
    toast({
      title: action === 'approve' ? "Creator Verified" : "Verification Rejected",
      description: `${creator?.creatorName} has been ${action === 'approve' ? 'assigned' : 'denied'} verified status.`,
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-secondary mb-1">
            ...
            <Text variant="label" className="text-[10px] font-bold tracking-widest uppercase">Compliance Hub</Text>
          </div>
          <Text variant="h1" className="text-3xl font-bold">Expert Authentication</Text>
          <Text variant="bodySmall" className="text-muted-foreground mt-1">
            Review and validate experts for the Imperialpedia Intelligence Index.
          </Text>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-secondary/10 border border-secondary/20 text-secondary">
          <ShieldCheck className="h-4 w-4" />
          <span className="text-xs font-bold uppercase tracking-wider">{requests.length} Pending Requests</span>
        </div>
      </header>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell colSpan={5} className="h-64 text-center">
                  <div className="flex flex-col items-center justify-center space-y-4">
                    ...
                    <Text variant="caption" className="animate-pulse">Validating credentials...</Text>
                  </div>
                </TableCell>
              </TableRow>
            ) : filtered.length === 0 ? (
              ...
              <TableRow key={req.creatorId} className="group hover:bg-muted/20">
                <TableCell className="py-4">
                  <div className="flex items-center gap-3">
                    <Avatar className="h-10 w-10 rounded-xl border border-white/10">
                      <AvatarImage src={req.creatorAvatar} />
                      <AvatarFallback>{req.creatorName.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div className="flex flex-col">
                      <span className="text-sm font-bold">{req.creatorName}</span>
                      <span className="text-[10px] text-muted-foreground">ID: {req.creatorId}</span>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex flex-wrap gap-1">
                    {req.documentsProvided.map(doc => (
                      <Badge key={doc} variant="outline" className="text-[9px] font-bold py-0 h-5 border-secondary/30 bg-secondary/5 text-secondary">
                ...
                <TableCell>
                  <div className="flex flex-col">
                    <span className="text-xs font-bold">{format(new Date(req.requestedAt), 'MMM d, yyyy')}</span>
                    <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                      <Clock className="h-2.5 w-2.5" /> {format(new Date(req.requestedAt), 'HH:mm')}
                    </span>
                  </div>
                </TableCell>
                <TableCell>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="glass-card bg-secondary/5 border-secondary/20 p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <Text variant="bodySmall" weight="bold">Verification Criteria</Text>
              <Text variant="caption" className="text-muted-foreground mt-1 leading-relaxed">
                Expert status requires proven industry experience, advanced academic degrees, or a significant public record of financial research. Always verify primary sources before approving.
              </Text>
            </div>
          </div>
        </Card>
        ...
        <Card className="glass-card border-primary/20 p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <Text variant="bodySmall" weight="bold">Platform Benefits</Text>
              <Text variant="caption" className="text-muted-foreground mt-1 leading-relaxed">
                Verified creators receive increased reach in the Intelligence Engine, early access to new AI content tools, and a higher revenue share from platform engagement.
              </Text>
            </div>
          </div>
        </Card>
