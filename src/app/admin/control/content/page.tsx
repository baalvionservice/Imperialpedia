                ...
                <TableRow key={item.content_id} className="group hover:bg-white/5 transition-colors border-b border-white/5">
                  ...
                  <TableCell className="text-right pr-8">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:text-primary transition-all"><ExternalLink className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:text-destructive transition-all"><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="glass-card border-none bg-primary/5 p-8 flex flex-col gap-4 relative overflow-hidden group">
          ...
          <div>
            ...
            <Text variant="caption" className="text-muted-foreground mt-2 leading-relaxed italic">
              "Research nodes created via the Admin Console bypass the standard author vetting queue and are prioritized for immediate pSEO ingestion."
            </Text>
          </div>
        </Card>
        ...
        <Card className="glass-card border-none bg-secondary/5 p-8 flex flex-col gap-4">
          <div>
            ...
            <Text variant="caption" className="text-muted-foreground mt-2 leading-relaxed">
              Every node commit is cryptographically signed and logged in the **Audit Trail**. Deleting a node initiates a recursive cache-burst across global CDN edge nodes.
            </Text>
            <Button variant="link" className="p-0 h-auto text-secondary text-xs font-bold mt-4 group" asChild>
              <Link href="/admin/control/audit-trail">
                View Access Logs <ArrowRight className="ml-1 h-3 w-3 transition-transform group-hover:translate-x-1" />
              </Link>
            </Button>
          </div>
        </Card>
