...
export function AccessManagementClient() {
  ...
  const handleToggleFeature = (name: string) => {
    ...
    setData({
      ...
      feature_flags: data.feature_flags.map((f) =>
        f.feature_name === name
          ? { ...f, status: f.status === "mock_on" ? "mock_off" : "mock_on" }
          : f
      ),
    });
    toast({
  ...
  const filteredFeatures = data.feature_flags.filter(
    (f) =>
      f.feature_name.toLowerCase().includes(search.toLowerCase()) ||
      f.description.toLowerCase().includes(search.toLowerCase())
  );

  const filteredAssignments = data.mock_user_assignments.filter(
    (a) =>
  ...
  return (
    <div className="space-y-10 pb-24 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <TabsList className="bg-transparent border-none">
              <TabsTrigger
                value="features"
                className="px-6 h-10 gap-2 rounded-xl font-bold text-xs data-[state=active]:bg-primary"
              >
            ...
            <div className="grid grid-cols-1 gap-4">
              {filteredFeatures.map((flag) => (
                <Card
                  ...
                  className="glass-card border-none hover:border-primary/20 transition-all group overflow-hidden"
                >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex gap-5 items-start">
                        ...
                        <div className="space-y-1">
                          ...
                          <Text
                            variant="caption"
                            className="text-muted-foreground block leading-relaxed max-w-md"
                          >
                            {flag.description}
                          </Text>
                        </div>
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right flex flex-col items-end gap-1.5">
                          <Text
                            variant="label"
                            className="text-[8px] opacity-50 font-bold uppercase tracking-widest"
                          >
                            Active
                          </Text>
                          <Switch
                            ...
                            onCheckedChange={() =>
                              handleToggleFeature(flag.feature_name)
                            }
                          />
                        </div>
                        <Button
                          ...
                          onClick={() => handleAction("Schedule Rollout")}
                        >
          ...
          <div className="lg:col-span-4 space-y-8">
            <Card className="glass-card border-none bg-primary/5 p-8 relative overflow-hidden group h-fit">
              ...
              <div className="space-y-6 relative z-10">
                ...
                >
                  "Feature flags are cryptographically signed. Activation
                  triggers an immediate cache-burst across global CDN edge
                  nodes."
                </Text>
              </div>
            </Card>
            ...
            <div className="p-8 rounded-[3rem] bg-secondary/5 border border-secondary/20 space-y-4">
              ...
              >
                Critical features like **Monetization** and **User Migration**
                require multi-administrator sign-off before being toggled in the
                production cluster.
              </Text>
              <Button
                variant="link"
                ...
                asChild
              >
                <button onClick={() => handleAction("Review Approval Chain")}>
      ...
      <TabsContent
        value="roles"
        className="mt-0 space-y-12 animate-in fade-in duration-500 outline-none"
      >
        <section className="space-y-8">
          <div className="flex items-center justify-between px-2">
            ...
            <Button
              ...
              onClick={() => handleAction("Define New Persona")}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {data.roles.map((role) => (
              <Card
                key={role.role_name}
                className="glass-card border-none shadow-xl hover:border-secondary/30 transition-all group"
              >
                <CardHeader className="p-6 pb-4">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <Badge
                      variant="outline"
                      className="border-white/10 text-[8px] font-bold uppercase tracking-widest bg-black/20"
                    >
                      System ID
                    </Badge>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {role.permissions.map((p) => (
                        <Badge
                          key={p}
                          variant="secondary"
                          className="bg-secondary/5 text-secondary border-none text-[8px] font-bold uppercase h-5 px-2"
                        >
                          {p.replace("_", " ")}
                        </Badge>
                      ))}
                ...
                <CardFooter className="p-4 bg-muted/10 border-t border-white/5 group-hover:bg-secondary/5 transition-colors">
                  <Button
                    ...
                    onClick={() => handleAction("Modify Permissions")}
                  >
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge
                          ...
                          className={cn(
                            ...
                            assign.assigned_role === "Admin"
                              ? "bg-primary/10 text-primary"
                              : assign.assigned_role === "Editor"
                      ...
                      <TableCell>
                        <div className="flex justify-center">
                          <Badge className="bg-emerald-500/10 text-emerald-500 border-none text-[8px] font-bold uppercase h-5 px-2">
                            Verified
                          </Badge>
                        </div>
                      </TableCell>
                      <TableCell className="text-right pr-8">
                        <div className="flex justify-end gap-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-8 px-4 rounded-xl text-[10px] font-bold uppercase gap-2 text-muted-foreground hover:text-primary transition-all"
                            onClick={() => handleAction("Change Persona")}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => handleAction("Unassign Role")}
                          >
        ...
        <Card className="glass-card border-none bg-primary/5 p-10 relative overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-10 relative z-10">
            ...
            <div className="flex-1 text-center lg:text-left space-y-2">
              ...
              >
                The platform architecture enforces **Least Privilege by
                Default**. Every persona shift is logged in the immutable audit
                trail. Changes propagate across the identity cluster within 180
                seconds.
              </Text>
            </div>
            <Button
