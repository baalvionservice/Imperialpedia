...
export default function SystemAlertsManagementPage() {
  ...
  const handleToggleStatus = (id: string, currentStatus: string) => {
    ...
    toast({
      title: newStatus === 'Active' ? "Alert Broadcast Active" : "Alert Node Sunset",
  ...
  const getTypeIcon = (type: SystemAlert['type']) => {
    switch (type) {
      case 'Info': return <Info className="h-3.5 w-3.5 text-primary" />;
    ...
    <div className="space-y-8 pb-24 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-primary mb-1">
              ...
              <Text variant="label" className="text-[10px] font-bold tracking-widest uppercase">Messaging Command</Text>
            </div>
            <Text variant="h1" className="text-3xl font-bold tracking-tight">System Alerts</Text>
          </div>
        </div>
            ...
            <form onSubmit={handleSendManualAlert}>
              <DialogHeader className="p-8 bg-primary/5 border-b border-white/5">
                ...
                <DialogDescription className="text-muted-foreground pt-2">
                  Draft a global broadcast signal for high-priority platform events.
                </DialogDescription>
              </DialogHeader>
              
              <div className="p-8 space-y-6">
                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Alert Headline / Payload</Label>
                  <Textarea 
                  ...
                  <div className="space-y-2">
                    ...
                    <Select defaultValue="all">
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="creators">Verified Experts Only</SelectItem>
                        <SelectItem value="admins">Infrastructure Staff</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
              ...
              <DialogFooter className="p-8 bg-muted/20 border-t border-white/5 gap-3">
                <Button type="button" variant="ghost" onClick={() => setIsModalOpen(false)}>Cancel Dispatch</Button>
                <Button type="submit" disabled={sending} className="h-12 px-8 rounded-xl font-bold bg-primary hover:bg-primary/90 shadow-xl shadow-primary/20">
                  {sending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Transmitting...</> : 'Broadcast Signal'}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Total Broadcasts</CardTitle>
            <Bell className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{alerts.length}</div>
            <p className="text-[10px] text-muted-foreground mt-1">Lifecycle total</p>
          </CardContent>
        </Card>

        <Card className="glass-card border-none shadow-xl bg-emerald-500/5">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Live Nodes</CardTitle>
            <CheckCircle2 className="h-4 w-4 text-emerald-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{alerts.filter(a => a.status === 'Active').length}</div>
            <p className="text-[10px] text-emerald-500 font-bold mt-1 uppercase tracking-tighter">Active Signals</p>
          </CardContent>
        </Card>

        <Card className="glass-card border-none shadow-xl bg-destructive/5">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Critical Triage</CardTitle>
        ...
        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Dispatch SLA</CardTitle>
            <Clock className="h-4 w-4 text-secondary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">120ms</div>
            <p className="text-[10px] text-muted-foreground mt-1">Avg. propagation time</p>
          </CardContent>
        </Card>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/20 hover:bg-muted/20 border-b border-white/5">
                <TableHead className="pl-6 font-bold text-[10px] uppercase tracking-widest py-4">Alert Signal Payload</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest">Classification</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center">Broadcast Status</TableHead>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={5} className="h-64 text-center">
                    ...
                    <Text variant="caption" className="mt-4 block animate-pulse font-bold tracking-widest uppercase">Synchronizing Alert Matrix...</Text>
                  </TableCell>
                </TableRow>
                ...
                <TableRow key={alert.id} className="group hover:bg-muted/10 transition-colors border-b border-white/5">
                  <TableCell className="py-5 pl-6">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold truncate max-w-[400px]">{alert.message}</span>
                      <span className="text-[9px] text-muted-foreground font-mono uppercase mt-1">Node ID: {alert.id}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-xs font-bold">{format(new Date(alert.date), 'MMM d, yyyy')}</span>
                  ...
                  <TableCell className="text-right pr-6">
                    <div className="flex justify-end gap-2">
                      <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-destructive transition-colors" onClick={() => handleDelete(alert.id)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="glass-card bg-primary/5 border-primary/20 p-6 flex flex-col gap-4">
          ...
          <div>
            <Text variant="bodySmall" weight="bold">Broadcast Integrity</Text>
            <Text variant="caption" className="text-muted-foreground mt-1 leading-relaxed">
              Every system alert is cryptographically signed. Activation triggers an immediate cache-bust across the **Intelligence CDN** to ensure real-time delivery.
            </Text>
          </div>
        </Card>
        ...
        <Card className="glass-card border-secondary/20 p-6 flex flex-col gap-4">
          ...
          <div>
            <Text variant="bodySmall" weight="bold">Priority Escalation</Text>
            <Text variant="caption" className="text-muted-foreground mt-1 leading-relaxed">
              Alerts classified as **Critical** are automatically pushed to the SMS Protocol for infrastructure staff and pinned to the global header for all users.
            </Text>
          </div>
        </Card>
        ...
        <Card className="glass-card border-emerald-500/20 p-6 flex flex-col gap-4">
          ...
          <div>
            <Text variant="bodySmall" weight="bold">Audit Retention</Text>
            <Text variant="caption" className="text-muted-foreground mt-1 leading-relaxed">
              Alert history is retained for **90 days** within the communication archive for post-incident reviews and editorial performance tracking.
            </Text>
          </div>
        </Card>
      </div>
