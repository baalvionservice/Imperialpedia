...
export default function TopKeywordsPage() {
  ...
  const avgCtr = (data.reduce((acc, curr) => acc + curr.ctr, 0) / data.length).toFixed(1);
    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <Text variant="h1" className="text-3xl font-bold tracking-tight">Keyword Analytics</Text>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Total Impressions</CardTitle>
        ...
        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Average CTR</CardTitle>
        ...
        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Top Rank</CardTitle>
            <TrendingUp className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">#1.2</div>
            <p className="text-[10px] text-emerald-500 font-bold mt-1">Dominant in 'Economics'</p>
          </CardContent>
        </Card>

        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Active Queries</CardTitle>
      ...
      <Card className="glass-card border-none shadow-2xl overflow-hidden">
        <CardHeader className="bg-card/30 border-b border-white/5 p-8 flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Aggregate impression velocity for the top 5 search queries.</CardDescription>
          </div>
          <Badge variant="outline" className="border-primary/20 bg-primary/5 text-primary text-[10px] font-bold uppercase tracking-widest">LIVE SYNC</Badge>
        </CardHeader>
      ...
      <Card className="glass-card border-none shadow-2xl overflow-hidden">
        <CardHeader className="bg-card/30 border-b border-white/5 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <CardDescription>Auditing individual keyword rankings and node-level conversion.</CardDescription>
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/20 hover:bg-muted/20 border-b border-white/5">
                <TableHead className="pl-6 font-bold text-[10px] uppercase tracking-widest">Search Query (Keyword)</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest">Discovery Node (Page)</TableHead>
              ...
              {filteredData.map((item) => (
                <TableRow key={item.id} className="group hover:bg-muted/10 transition-colors border-b border-white/5">
                  <TableCell className="py-5 pl-6">
                    <span className="text-sm font-bold text-foreground">{item.keyword}</span>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right pr-6">
                    <div className="flex justify-end items-center gap-2">
                      <Badge className={cn(
                        ...
                        item.trend === 'Up' ? "bg-emerald-500/10 text-emerald-500" : "bg-destructive/10 text-destructive"
                      )}>
                        {item.trend === 'Up' ? <ArrowUpRight className="h-2.5 w-2.5 mr-1" /> : <ArrowDownRight className="h-2.5 w-2.5 mr-1" />}
                        {item.trend}
                      </Badge>
                    </div>
                  </TableCell>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="glass-card bg-primary/5 border-primary/20 p-8 flex flex-col gap-4">
          ...
          <div>
            <Text variant="h3" className="mb-2 text-xl font-bold">E-E-A-T Signal Audit</Text>
            <Text variant="bodySmall" className="text-muted-foreground leading-relaxed">
              We identified a **15% lift in rankings** for nodes associated with verified expert profiles. Programmatic nodes in the 'Investing' hub require additional citation depth to maintain Q2 positions.
            </Text>
          </div>
        </Card>
        ...
        <Card className="glass-card border-secondary/20 p-8 flex flex-col gap-4">
          ...
          <div>
            <Text variant="h3" className="mb-2 text-xl font-bold">Search Intent Analysis</Text>
            <Text variant="bodySmall" className="text-muted-foreground leading-relaxed">
              Queries for **'Compound Interest Engine'** show 2.4x higher conversion than standard articles. Recommend cross-linking from all basic finance definitions to interactive tools.
            </Text>
          </div>
        </Card>
      </div>
