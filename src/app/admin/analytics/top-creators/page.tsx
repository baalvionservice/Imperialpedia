    ...
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 text-primary mb-1">
              ...
              <Text variant="label" className="text-[10px] font-bold tracking-widest uppercase">Elite Network Audit</Text>
            </div>
            <Text variant="h1" className="text-3xl font-bold">Top Performing Creators</Text>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Total Network Reach</CardTitle>
            <Users className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatCompact(data.reduce((acc, c) => acc + c.followers, 0))}</div>
        ...
        <Card className="glass-card border-none shadow-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Expert Accrual</CardTitle>
            <DollarSign className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(data.reduce((acc, c) => acc + c.revenue, 0))}</div>
        ...
        <Card className="lg:col-span-8 glass-card border-none shadow-2xl overflow-hidden">
          <CardHeader className="bg-card/30 border-b border-white/5 p-6 flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Aggregate impression velocity across the top 10 experts.</CardDescription>
            </div>
          ...
          <Card className="glass-card border-none shadow-xl bg-primary/5">
            ...
            <CardContent>
              <Text variant="caption" className="italic text-muted-foreground leading-relaxed">
                "Experts publishing in the **Fixed Income** taxonomy are seeing a 45% higher share-to-view ratio. We recommend featuring these creators in the Q2 Intelligence Highlights."
              </Text>
              <Button variant="link" className="p-0 text-primary h-auto text-xs font-bold mt-4" asChild>
                <Link href="/admin/analytics/content">View Taxonomy Audit <ChevronRight className="ml-1 h-3 w-3" /></Link>
              </Button>
            </CardContent>
          </Card>
              ...
              {data.slice(0, 3).map((creator, i) => (
                <div key={creator.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <Text variant="bodySmall" weight="bold" className="truncate max-w-[120px]">{creator.name}</Text>
                  </div>
                  <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-500 border-none font-mono text-[10px]">
      ...
      <Card className="glass-card border-none shadow-2xl overflow-hidden">
        <CardHeader className="bg-card/30 border-b border-white/5 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <CardTitle className="text-xl">Expert Performance Leaderboard</CardTitle>
            <CardDescription>Auditing individual impact and network expansion.</CardDescription>
          </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/20 hover:bg-muted/20 border-b border-white/5">
                <TableHead className="w-16 text-center font-bold text-[10px] uppercase tracking-widest">Rank</TableHead>
                ...
                <TableHead className="font-bold text-[10px] uppercase tracking-widest text-right">Audience</TableHead>
                <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center">Engagement</TableHead>
                <TableHead className="text-right pr-6 font-bold text-[10px] uppercase tracking-widest">Revenue Impact</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                <TableRow key={creator.id} className="group hover:bg-muted/10 transition-colors border-b border-white/5">
                  ...
                  <TableCell className="py-5 pl-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-muted-foreground uppercase tracking-tighter">Verified Creator</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 text-[8px] font-bold uppercase h-5">
                      {creator.category}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-right pr-6">
                    <div className="flex flex-col items-end">
                      <span className="text-xs font-bold text-emerald-500">{formatCurrency(creator.revenue)}</span>
                      <span className="text-[9px] text-muted-foreground uppercase">Lifetime Earned</span>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
