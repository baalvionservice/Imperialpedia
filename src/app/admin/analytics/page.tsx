  ...
  if (loading || !data) {
    return (
      <div className="py-40 flex flex-col items-center justify-center space-y-4">
        ...
        <Text
          variant="bodySmall"
          className="text-muted-foreground animate-pulse font-bold uppercase tracking-widest"
        >
    ...
    <div className="space-y-8 pb-24 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <Text variant="bodySmall" className="text-muted-foreground mt-1">
            Visualizing growth, engagement, and content performance across the
            Imperialpedia index.
          </Text>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <TabsList className="bg-card/30 border border-white/5 h-11 px-1 rounded-xl">
              <TabsTrigger
                value="weekly"
                className="rounded-lg px-4 font-bold text-[10px] uppercase"
              >
                Weekly
              </TabsTrigger>
              <TabsTrigger
          ...
          <Button
            variant="outline"
            className="h-11 px-6 rounded-xl border-white/10 bg-card/30 gap-2 font-bold text-xs"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="glass-card border-none shadow-xl bg-primary/5 group hover:border-primary/20 transition-all">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {data.user_activity[data.user_activity.length - 1].active_users}{" "}
              Active
            </div>
            <p className="text-[10px] text-emerald-500 font-bold mt-1 flex items-center gap-1">
              <ArrowUpRight className="h-2.5 w-2.5" /> +
              {data.user_activity[data.user_activity.length - 1].new_signups}{" "}
              signups today
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="glass-card border-none shadow-xl">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCompact(data.engagement_metrics.comments)} Comments
            </div>
            <div className="flex items-center gap-3 mt-1">
              <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                <ArrowBigUp className="h-2.5 w-2.5" />{" "}
                {formatCompact(data.engagement_metrics.upvotes)}
              </span>
              <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                <Zap className="h-2.5 w-2.5" />{" "}
                {formatCompact(data.engagement_metrics.shares)}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="glass-card border-none shadow-xl bg-emerald-500/5">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(
                data.revenue_metrics[data.revenue_metrics.length - 1].amount
              )}
            </div>
            <p className="text-[10px] text-emerald-500 font-bold mt-1 flex items-center gap-1">
        ...
        <Card className="lg:col-span-8 glass-card border-none shadow-2xl overflow-hidden">
          <CardHeader className="bg-card/30 border-b border-white/5 p-8 flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>
                Correlation between active users and new registrations.
              </CardDescription>
            </div>
            <Badge
              variant="outline"
              className="border-primary/20 bg-primary/5 text-primary text-[10px] font-bold uppercase tracking-widest px-3"
            >
              Live Stream
            </Badge>
          </CardHeader>
          ...
          <Card className="glass-card border-none shadow-xl h-full flex flex-col">
            ...
            <CardContent className="p-6 flex-grow flex flex-col justify-center">
              ...
              <div className="mt-6 p-4 rounded-xl bg-secondary/5 border border-secondary/20">
                ...
                >
                  "Watchlist Hub adoption is up 45% following the deployment of
                  the real-time sentiment node."
                </Text>
              </div>
            </CardContent>
          </Card>
        ...
        <CardHeader className="bg-card/30 border-b border-white/5 p-8 flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-xl flex items-center gap-2">
              <FileText className="h-5 w-5 text-primary" /> Intelligence Report
              Matrix
            </CardTitle>
            <CardDescription>
              Auditing individual content node impact and conversion.
            </CardDescription>
          </div>
          <Button
            variant="ghost"
            size="sm"
            className="text-primary font-bold text-xs group"
          >
            View All Content{" "}
              ...
              {data.content_reports.map((report) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className="bg-primary/5 text-primary border-primary/20 text-[8px] font-bold uppercase"
                    >
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1 text-emerald-500 font-bold text-xs">
                      ...
                      {formatCompact(report.likes)} likes
                    </div>
                  </TableCell>
                  <TableCell className="text-right pr-8">
                    <Badge className="bg-emerald-500/10 text-emerald-500 border-none text-[9px] font-bold uppercase px-3 h-6">
                      Optimized
                    </Badge>
                  </TableCell>
                </TableRow>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10">
          <div className="lg:col-span-4 flex flex-col justify-center space-y-4">
            ...
            <div>
              ...
              >
                Monitoring high-fidelity platform accrual across institutional
                and expert member tiers.
              </Text>
            </div>
