...
export function TopicIndexClient({ initialData }: TopicIndexClientProps) {
  ...
  const groupedAlphabetical = useMemo(() => {
    ...
    'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').forEach(letter => {
      const topics = filteredTopics.filter(t => t.title.charAt(0).toUpperCase() === letter);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-8 space-y-16">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-10">
            <TabsList className="bg-card/30 border border-white/5 p-1 h-14 rounded-2xl w-full md:w-auto justify-start">
              <TabsTrigger value="alphabetical" className="px-8 h-12 gap-2 text-xs font-bold rounded-xl data-[state=active]:bg-primary uppercase tracking-widest">
                <SortAsc className="h-4 w-4" /> A – Z Index
              </TabsTrigger>
              <TabsTrigger value="categories" className="px-8 h-12 gap-2 text-xs font-bold rounded-xl data-[state=active]:bg-primary uppercase tracking-widest">
                <Layers className="h-4 w-4" /> Category Hubs
              </TabsTrigger>
              <TabsTrigger value="paths" className="px-8 h-12 gap-2 text-xs font-bold rounded-xl data-[state=active]:bg-primary uppercase tracking-widest">
                <GraduationCap className="h-4 w-4" /> Learning Paths
              </TabsTrigger>
            </TabsList>

            {/* ALPHABETICAL INDEX CONTENT */}
            <TabsContent value="alphabetical" className="mt-0 space-y-12 animate-in fade-in duration-500">
              ...
              <div className="space-y-16">
                {Object.entries(groupedAlphabetical).map(([letter, topics]) => (
                  <div key={letter} id={`letter-${letter}`} className="scroll-mt-48 space-y-8">
                    <div className="flex items-center gap-6">
                      ...
                      <Text variant="label" className="text-[10px] text-muted-foreground font-bold">{topics.length} Nodes</Text>
                    </div>
                          ...
                          <Card className="glass-card border-none hover:border-primary/30 transition-all duration-300 group overflow-hidden h-full">
                            <CardContent className="p-6 space-y-4">
                              <div className="flex justify-between items-start">
                                <Badge variant="outline" className="text-[8px] font-bold uppercase border-white/10 bg-black/20 text-muted-foreground">{topic.category}</Badge>
                              ...
                              <Text variant="body" weight="bold" className="group-hover:text-primary transition-colors text-lg">{topic.title}</Text>
                              <Text variant="caption" className="text-muted-foreground line-clamp-2 leading-relaxed italic">"{topic.definition}"</Text>
                              
                              <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                                <div className="flex items-center gap-4 text-[9px] text-muted-foreground font-bold uppercase tracking-tighter">
                                  <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {topic.reading_time}</span>
                                  <span className="flex items-center gap-1"><Activity className="h-3 w-3" /> {topic.difficulty}</span>
                                </div>
                                <div className="flex items-center gap-1 text-primary">
                                  <TrendingUp className="h-3 w-3" />
                                  <span className="text-[10px] font-mono font-bold">{topic.popularity_score}</span>
                                </div>
                              </div>
                            </CardContent>
                          </Card>
            ...
            <TabsContent value="categories" className="mt-0 space-y-10 animate-in fade-in duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {initialData.categories.map((cat) => (
                  <Card key={cat.name} className="glass-card border-none shadow-xl hover:border-primary/30 transition-all duration-500 group overflow-hidden">
                    <CardContent className="p-8">
                      <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="text-right">
                          <div className="text-3xl font-bold tracking-tighter">{cat.count}</div>
                          <Text variant="label" className="text-[9px] opacity-50 font-bold uppercase tracking-widest">Indexed Nodes</Text>
                        </div>
                      </div>
                      <Text variant="h3" className="text-2xl font-bold mb-4">{cat.name}</Text>
                      <Button variant="ghost" className="p-0 h-auto text-primary font-bold text-xs group/btn" asChild>
                        <Link href={`/categories/${cat.name.toLowerCase().replace(' ', '-')}`}>
                          Explore Hub <ChevronRight className="ml-1 h-4 w-4 transition-transform group-hover/btn:translate-x-1" />
                        </Link>
                      </Button>
                    </CardContent>
                  </Card>
            ...
            <TabsContent value="paths" className="mt-0 space-y-10 animate-in fade-in duration-500">
              {initialData.learning_paths.map((path) => (
                <Card key={path.id} className="glass-card border-none bg-primary/5 shadow-2xl relative overflow-hidden group">
                  ...
                  <CardHeader className="p-10 border-b border-white/5 space-y-4">
                    <Badge variant="secondary" className="bg-primary/10 text-primary border-none text-[9px] font-bold uppercase h-6 px-3 tracking-widest">
                      Curated Intelligence Node
                    </Badge>
                    <CardTitle className="text-3xl font-bold">{path.name}</CardTitle>
                  ...
                  <CardFooter className="p-10 pt-0">
                    <Button size="lg" className="rounded-xl h-12 px-10 font-bold bg-primary hover:bg-primary/90 shadow-xl shadow-primary/20">
        ...
        <aside className="lg:col-span-4 space-y-10">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-white/5">
                {initialData.trending_topics.map((topic, idx) => (
                  <div key={topic} className="p-5 hover:bg-white/5 transition-all group cursor-pointer flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] font-mono font-bold text-muted-foreground opacity-50 w-4">#{idx + 1}</span>
                      <Text variant="bodySmall" weight="bold" className="group-hover:text-primary transition-colors">{topic}</Text>
                    </div>
          ...
          <div className="p-10 rounded-[3rem] bg-secondary/5 border border-secondary/20 space-y-6 relative overflow-hidden group">
            ...
            <Text variant="h3" className="font-bold">Scale Telemetry</Text>
            ...
            <Text variant="caption" className="text-muted-foreground leading-relaxed italic block">
              "The Imperialpedia index is synchronized across global nodes every 60 seconds to ensure discovery of new market intelligence."
            </Text>
          </div>
          ...
          <div className="p-8 rounded-[3rem] bg-card/30 border border-white/5 text-center space-y-4">
            ...
            <Text variant="bodySmall" weight="bold">Missing a specialized node?</Text>
            <Text variant="caption" className="text-muted-foreground leading-relaxed block">
              Our expert community can architect new taxonomy hubs for specialized assets or complex derivatives.
            </Text>
            <Button variant="outline" className="w-full h-12 rounded-2xl border-primary/20 text-primary font-bold text-xs uppercase">
